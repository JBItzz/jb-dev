...
export default function Portfolio() {
  ...
  const levelColor = (level) => {
    if (level === "Advanced") return "bg-emerald-500/10 text-emerald-400 border-emerald-500/20";
    if (level === "Intermediate") return "bg-blue-500/10 text-blue-400 border-blue-500/20";
    return "bg-neutral-500/10 text-neutral-400 border-neutral-500/10";
  };
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            {skills.map((skill, index) => (
              <div 
                key={index} 
                className={`group relative flex flex-col justify-between p-3 rounded-xl border transition-all duration-300 ease-out hover:-translate-y-0.5 shadow-sm
                  ${isDarkMode 
                    ? 'bg-neutral-900/40 border-neutral-800/60 hover:bg-neutral-900 hover:border-neutral-700 hover:shadow-neutral-950/50' 
                    : 'bg-white border-neutral-200 hover:bg-neutral-50 hover:border-neutral-300 hover:shadow-neutral-200/50'}`}
              >
                {/* Ambient glow on hover */}
                <div className="absolute inset-0 rounded-xl opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none bg-gradient-to-br from-neutral-500/5 to-transparent" />
                
                <div className="flex items-start justify-between relative z-10">
                  <div className={`w-9 h-9 rounded-lg flex items-center justify-center border transition-all duration-300 group-hover:rotate-6
                    ${isDarkMode ? 'bg-neutral-950 border-neutral-800 text-neutral-400 group-hover:text-white' : 'bg-neutral-50 border-neutral-200 text-neutral-500 group-hover:text-neutral-900'}`}
                  >
                    {skill.icon}
                  </div>
                  <span className={`text-[10px] px-2 py-0.5 rounded-full font-semibold tracking-wide border uppercase scale-95 origin-right transition-transform group-hover:scale-100 ${levelColor(skill.level)}`}>
                    {skill.level}
                  </span>
                </div>

                <div className="mt-4 relative z-10">
                  <h3 className={`font-semibold text-sm transition-colors ${isDarkMode ? 'text-neutral-200 group-hover:text-white' : 'text-neutral-800 group-hover:text-neutral-900'}`}>
                    {skill.name}
                  </h3>
                </div>
              </div>
